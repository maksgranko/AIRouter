...
logging.basicConfig(level=logging.ERROR, format='%(levelname)s:%(name)s:%(asctime)s:%(message)s') # Изменено на DEBUG и добавлен asctime
logger = logging.getLogger(__name__) # Создаем логгер для main.py
logger.setLevel(logging.ERROR) # Убедимся, что и этот логгер на DEBUG
...
gemini_module_logger.setLevel(logging.ERROR)
